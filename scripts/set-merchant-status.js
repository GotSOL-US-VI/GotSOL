...
const idl = require("../src/utils/gotsol.json");
const fs = require('fs');
const os = require('os');
  ...
  try {
    ...
    try {
      ...
      if (!merchantAccount.feeEligible) {
        console.log("\n✅ Merchant is already fee ineligible (false)");
        return;
      }
    ...
    console.log("\n🔄 Setting fee_eligible to false...");
    
    // Call the set_merchant_status instruction
    const tx = await program.methods
      .setMerchantStatus(false) // Set fee_eligible to false
      .accounts({
    ...
    if (!updatedMerchant.feeEligible) {
      console.log("🎉 Successfully set merchant fee_eligible to false!");
    } else {
      console.log("⚠️  Warning: fee_eligible is still true");
    }
